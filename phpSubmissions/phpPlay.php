<?php 
    ...
    //0 - Check if is reading
    $check_is_reading = "SELECT `is_reading` FROM `clubs` WHERE `name` = '".$club."'";
    if(mysqli_fetch_row($conn->query($check_is_reading))[0] == 1)
    {
       echo ("W8-");
       die;
    }
    ...
    {
        $conn->query("UPDATE `clubs` SET `is_reading` = 1 WHERE `name` = '".$club."'");
    ...
    $conn->query("UPDATE `clubs` SET `is_reading` = 0 WHERE `name` = '".$club."'");
?>